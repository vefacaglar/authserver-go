...
> **How to use this file.** This is a self-contained build prompt for an LLM working in a *fresh, empty Go repository*. It is a port of an existing, production-hardened C# OAuth2/OIDC server (`Vefa.CustomAuth`) into idiomatic Go. The **target is a standalone runnable auth server**, not a reusable library/NuGet-style package. You do not have access to the C# source; everything you need is specified below. Build the protocol flow first with in-memory stores, prove it, then add GORM persistence behind the same interfaces.
...
**Language rule (strict):** all code identifiers, comments, log messages, error strings, user-facing protocol error responses, commit messages, and `.md` docs are **English only**. Chat replies to the author may be Turkish; nothing shipped in the repo may be.
...
## Build milestones (do them in order; keep `go build ./...` + `go vet ./...` clean at each)

1. **M1 — Module + domain + memory stores.** `go.mod`, `internal/domain`, `internal/store` interfaces, `internal/store/memory` with atomic `MarkConsumed` (per-entity mutex), `Clock` abstraction. No HTTP yet.
2. **M2 — Token core.** `internal/token` (hasher, pkce, keys with RSA bootstrap, issuer with at_hash/nonce). Unit tests for hashing, PKCE S256, JWT round-trip, at_hash.
3. **M3 — Core OIDC flow (in-memory).** discovery, jwks, authorize (with redirect-first validation + session/prompt/max_age), token dispatcher + authcode grant, session cookie, login GET/POST, errors helper, router wiring in `internal/server` + `cmd/authserver`. **Prove Authorization Code + PKCE end-to-end** with an httptest integration test before touching the DB.
4. **M4 — Refresh + remaining endpoints.** refresh grant (rotation + reuse detection), userinfo (scope-filtered), revoke, logout (RP-initiated + confirm page). Integration tests for rotation, reuse-detection chain revocation, single-use code, expired code, bad PKCE.
5. **M5 — GORM persistence.** `internal/store/gormstore` for all stores with `AutoMigrate`; atomic CAS via conditional `Update`; Postgres + SQLite drivers. Run the same integration suite against SQLite. Switch `cmd/authserver` to GORM (driver from config), keep memory store available for tests.
6. **M6 — Confidential clients.** client_credentials grant + `private_key_jwt` client auth + jti replay cache; discovery advertises them. Tests: end-to-end client_credentials exchange, forged-key rejection, replay rejection.
7. **M7 — Admin UI/API.** auth-required chi group, JSON CRUD, CSRF on mutations, key-private-stripping, SPA page. Tests for the auth-required group (anonymous request rejected unless `AdminAllowAnonymous`).
8. **M8 — Hardening pass.** Run through the Security checklist; add rate limiting + lockout; security headers middleware; seed data (a demo client + scopes + sample user) for a one-command local run.
...
- Keep everything `internal/` — this is an application, not a library. One exported type per file isn't required in Go, but keep files focused and named after their primary type/function.
