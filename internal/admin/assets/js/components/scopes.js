...
function scopeForm() {
  return `
    <div class="flex items-center justify-between">
      <h3 class="text-lg font-bold tracking-tight">Create OIDC Scope</h3>
    ...
    <form id="scope-form" class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 md:p-8 shadow-sm space-y-5 max-w-xl mt-4">
      <div class="space-y-1">
        ...
        <input id="f-scope-name" required placeholder="e.g. read_profile" class="w-full px-3 py-2 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 dark:text-white">
      </div>
      <div class="space-y-1">
        <label for="f-scope-display" class="text-xs font-semibold text-zinc-400 uppercase tracking-wider block">Display Name</label>
        <input id="f-scope-display" placeholder="e.g. Read Profile Details" class="w-full px-3 py-2 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 dark:text-white">
      </div>
      <div class="space-y-1">
        <label for="f-scope-desc" class="text-xs font-semibold text-zinc-400 uppercase tracking-wider block">Description</label>
        <input id="f-scope-desc" placeholder="Allows the OIDC client to read profile metadata" class="w-full px-3 py-2 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 dark:text-white">
      </div>
      <div class="flex items-center gap-3 pt-2">
        <label class="flex items-center gap-2 text-sm cursor-pointer">
          <input id="f-scope-req" type="checkbox" class="rounded border-zinc-300 dark:border-zinc-700 text-indigo-600 focus:ring-indigo-500">
      ...
      <div class="flex items-center justify-end gap-2 pt-4 border-t border-zinc-100 dark:border-zinc-800 mt-4">
        ...
        <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-semibold shadow-md shadow-indigo-500/10 transition-colors">Create Scope</button>
      </div>
